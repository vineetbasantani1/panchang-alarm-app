...
/// Minimal metadata type required by AlarmKit's generic AlarmConfiguration<Metadata>.
/// AlarmMetadata's exact protocol requirements weren't confirmed by a compiler
/// error yet (the error we got was about the missing `attributes:` parameter,
/// not about this type's conformance) — if this doesn't satisfy AlarmMetadata,
/// Xcode's error will say so and point to what's missing.
@available(iOS 26.0, *)
struct PanchangAlarmMetadata: AlarmMetadata {}
#endif
...
final class AlarmScheduler: ObservableObject {
    ...
    // MARK: - AlarmKit path (round 2 — now using the REAL signature the
    // compiler revealed: AlarmManager.AlarmConfiguration<Metadata> requires
    // an `attributes: AlarmAttributes<Metadata>` parameter, where Metadata
    // is a type you define conforming to AlarmMetadata. The exact shape of
    // AlarmAttributes/AlarmPresentation below is still a best-effort guess
    // from WWDC session notes — expect another round of compiler fixes here,
    // same iterative process that got AlarmConfiguration right.
    ...
    private func scheduleWithAlarmKit(
        ...
        for match in matches {
            ...
            do {
                ...
                let attributes = AlarmAttributes<PanchangAlarmMetadata>(
                    presentation: presentation,
                    metadata: PanchangAlarmMetadata(),
                    tintColor: .orange
                )
                let configuration = AlarmManager.AlarmConfiguration<PanchangAlarmMetadata>(
